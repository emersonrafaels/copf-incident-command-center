...
INSERT INTO public.occurrences (
  ...
  status_equipamento, motivo_ocorrencia, motivo_impedimento
) 
SELECT 
  ...
  (ARRAY['pendente', 'em_andamento', 'resolvida', 'com_impedimentos'])[ceil(random() * 4)] as status,
  ...
  (ARRAY['convencional', 'digital', 'terceirizada', 'prime', 'tradicional'])[ceil(random() * 5)] as tipo_agencia,
  ...
  CASE 
    WHEN random() > 0.9 THEN (ARRAY['Aguardando peça', 'Falta de acesso', 'Autorização pendente', 'Problema técnico'])[ceil(random() * 4)]
    ELSE NULL
  END as motivo_impedimento
